...
#[allow(dead_code)]
fn benchmark_sgd_with_momentum_optimizer_small(c: &mut Criterion) {
    ...
    c.bench_function("sgd_with_momentum_optimizer_small", |b| {
        b.iter(|| {
            let mut optimizer = SGDWithMomentum::new(black_box(0.01), black_box(0.9));
...
#[allow(dead_code)]
fn benchmark_sgd_with_momentum_optimizer_large(c: &mut Criterion) {
    let mut rng = rand::thread_rng();
