...
use deltaml::optimizers::{MiniBatchGD, Optimizer};
use rand::Rng;

#[allow(dead_code)]
fn benchmark_minibatch_gd_optimizer_small(c: &mut Criterion) {
    ...
    c.bench_function("minibatch_gd_optimizer_small", |b| {
        b.iter(|| {
            let mut optimizer = MiniBatchGD::new(black_box(0.01));
...
#[allow(dead_code)]
fn benchmark_minibatch_gd_optimizer_large(c: &mut Criterion) {
    let mut rng = rand::thread_rng();
    ...
    let mut group = c.benchmark_group("MiniBatchGDOptimizer");
    group.measurement_time(std::time::Duration::new(10, 0));
    group.sample_size(40);

    group.bench_function("minibatch_gd_optimizer_large", |b| {
        b.iter(|| {
            let mut optimizer = MiniBatchGD::new(black_box(0.01));
            let mut weights_clone = weights.clone();
            let gradients_clone = gradients.clone();
...
criterion_group!(
    benches,
    benchmark_minibatch_gd_optimizer_small,
    benchmark_minibatch_gd_optimizer_large
);
