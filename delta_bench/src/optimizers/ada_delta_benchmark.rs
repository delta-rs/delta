...
fn benchmark_ada_delta_optimizer_small(c: &mut Criterion) {
    ...
    c.bench_function("ada_delta_optimizer_small", |b| {
        b.iter(|| {
            let mut optimizer = AdaDelta::new(black_box(0.9), black_box(1e-6));
...
#[allow(dead_code)]
fn benchmark_ada_delta_optimizer_large(c: &mut Criterion) {
    let mut rng = rand::thread_rng();
    ...
    let mut group = c.benchmark_group("AdaDeltaOptimizer");
    group.measurement_time(std::time::Duration::new(10, 0));
    group.sample_size(40);

    group.bench_function("ada_delta_optimizer_large", |b| {
        b.iter(|| {
            let mut optimizer = AdaDelta::new(black_box(0.9), black_box(1e-6));
            let mut weights_clone = weights.clone();
            let gradients_clone = gradients.clone();
...
criterion_group!(benches, benchmark_ada_delta_optimizer_small, benchmark_ada_delta_optimizer_large);
