...
fn benchmark_adagrad_optimizer_small(c: &mut Criterion) {
    ...
    c.bench_function("adagrad_optimizer_small", |b| {
        b.iter(|| {
            let mut optimizer = AdaGrad::new(black_box(0.1), black_box(1e-8));
...
fn benchmark_adagrad_optimizer_large(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("AdaGradOptimizer");
    group.measurement_time(std::time::Duration::new(10, 0));
    group.sample_size(40);

    group.bench_function("adagrad_optimizer_large", |b| {
        b.iter(|| {
            let mut optimizer = AdaGrad::new(black_box(0.1), black_box(1e-8));
            let mut weights_clone = weights.clone();
            let gradients_clone = gradients.clone();
...
criterion_group!(benches, benchmark_adagrad_optimizer_small, benchmark_adagrad_optimizer_large);
