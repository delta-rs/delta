...
#[allow(dead_code)]
fn benchmark_sgd_optimizer_small(c: &mut Criterion) {
    ...
    c.bench_function("sgd_optimizer_small", |b| {
        b.iter(|| {
            let mut optimizer = SGD::new(black_box(0.01));
...
#[allow(dead_code)]
fn benchmark_sgd_optimizer_large(c: &mut Criterion) {
    let mut rng = rand::thread_rng();
    ...
    let mut group = c.benchmark_group("SGDOptimizer");
    group.measurement_time(std::time::Duration::new(10, 0));
    group.sample_size(40);

    group.bench_function("sgd_optimizer_large", |b| {
        b.iter(|| {
            let mut optimizer = SGD::new(black_box(0.01));
            let mut weights_clone = weights.clone();
            let gradients_clone = gradients.clone();
